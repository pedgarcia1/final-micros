 ...
 ******************************************************************************/
<<<<<<< HEAD
#define BUFFER_SIZE 128  // Tama�o del buffer circular

typedef struct {
    uint8_t data[BUFFER_SIZE];  // Array para almacenar los datos recibidos
    uint8_t head;               // �ndice del pr�ximo byte a escribir
    uint8_t tail;               // �ndice del pr�ximo byte a leer
    uint8_t count;              // N�mero de bytes en el buffer
    uint8_t receiving;          // Bandera de estado de recepci�n
